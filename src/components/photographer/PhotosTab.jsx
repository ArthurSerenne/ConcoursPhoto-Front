...
const PhotoTab = () => {
  ...
  const totalPages = Math.ceil(photos.filter((photo) => photo.status === true).length / itemsPerPage);
  ...
  return (
    <div>
        <div className="mx-auto mt-10 mb-10 grid grid-cols-1 md:gap-6 items-center 2xl:max-w-screen-2xl xl:max-w-screen-xl lg:max-w-screen-lg md:max-w-screen-md sm:max-w-screen-sm">
            <p className='text-2xl'>{photos.length} photo{photos.length > 1 ? 's' : ''}</p>
            {loading ? 
                ...
                <div className='grid gap-6 md:grid-cols-3'>
                    {photos ? photos.map((photo) => <ImageDisplay imageName={photo} />) : ''} 
                </div>
            }
        </div>
        <div>
