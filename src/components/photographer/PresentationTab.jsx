        ...
        <div className='grid grid-cols-2'>
            <div className='flex flex-col justify-center md:flex-row gap-24 md:justify-start'>
                <div className='space-y-2'>
                    ...
                    <p>Catégorié photographe : {CategoryEnum[photographer?.member?.category]}</p>
                    <p>Tél : {photographer?.phone}</p>
                    <p className='pb-10'>Email : <span className='font-bold underline'>{photographer?.email}</span></p>
                    <a className='text-white bg-gray-400 px-9 py-5 rounded-full hover:bg-gray-300' href={photographer?.member?.website ? photographer?.member?.website : ''} target='_blank'>Site Web</a>
                </div>
                <div>
                    <img src={imageSrc} className='w-32 h-32 rounded-full' onError={handleError} alt="Photographer"/>
                </div>
            </div>
            <div className='space-y-2'>
                <p className='font-bold text-xl'>Dernières photos soumises</p>
                <div className='grid grid-cols-2 gap-4'>
                    {photographer?.member?.photos?.filter((photo) => photo.status === true).sort((a, b) => new Date(b.submissionDate) - new Date(a.submissionDate)).map((photo) => <ImageDisplay imageName={photo} radius={'rounded-lg'} />)}
                </div>
            </div>
        </div>
    );
