...
const FinishedContestsTab = () => {
    ...
    const paginatedContests = sortedContests.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage);
        ...
        .then((res) => {
          ...
          if (contests && contests.length > 0) {
            const activeContests = contests
              .filter((contest) => contest.deletionDate === undefined && contest.status === true && new Date(contest.resultsDate) < today)
    ...
    return (
        <div>
            <div className="mx-auto mt-10 mb-10 grid grid-cols-1 md:gap-6 items-center 2xl:max-w-screen-2xl xl:max-w-screen-xl lg:max-w-screen-lg md:max-w-screen-md sm:max-w-screen-sm">
                <p className='text-2xl'>{sortedContests.length} concours terminé{sortedContests.length > 1 ? 's' : ''}</p>
                {loading ? 
                    <div> 
                    ...
                    (paginatedContests ? paginatedContests.map((contest, index) => <ContestCardList key={index} contest={contest} />) : '')
                }
